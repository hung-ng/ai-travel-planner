...
# Check ChromaDB logs
docker logs travel_planner_chromadb

# Restart ChromaDB
docker-compose -f docker-compose.backend.yml restart chromadb
```
...
# Connect to database
docker exec -it travel_planner_postgres psql -U admin -d travel_planner
...
- **Missing OpenAI API key**: Ensure `OPENAI_API_KEY` is set in `backend/.env`
- **Rate limiting**: Data collection implements rate limiting automatically
- **Failed cities**: Check `progress.json` for failed cities and error details
- **Reprocess cities**: Use `--force` flag to reprocess all cities

### Test Failures

- **Mock not configured**: Ensure `conftest.py` fixtures are imported
- **Database errors**: Tests use SQLite in-memory; check test isolation
- **API mocking**: Verify OpenAI/ChromaDB mocks are active
...
- Extracted context tracked separately to avoid redundancy

### API Cost Management
- All tests use mocked OpenAI calls (no cost)
- Development uses gpt-4.1-nano (lowest cost GPT-4 model)
- Embeddings cached in ChromaDB (one-time cost per document)

### Database Performance
- PostgreSQL indexes on frequently queried fields
- ChromaDB uses HNSW algorithm for fast similarity search
...
- All open data sources (Wikipedia, Wikivoyage, OpenStreetMap, etc.)
